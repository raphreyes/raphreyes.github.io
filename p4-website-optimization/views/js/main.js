...
var resizePizzas = function(size) {
  ...
  // Replaced all querySelector in this funciton with getElementById
...
var pizzaSize = changeSliderLabel(size);

// I have deleted an unecessary changeSliderLabel(size); since we set a size in changeSliderLabel(size)
// changeSliderLabel(size) deleted.

  // Go through pizza elements on the page and change their widths
  // We assign the pizza elements to a variable allPizza, and the length
  // of the allPizza array to sizeLoop variable so we can iterate through the elements
  var allPizza = document.getElementsByClassName('randomPizzaContainer');
  var sizeLoop = allPizza.length;
  function changePizzaSizes(size) {
    for (var i = 0; i < sizeLoop; i++) {
      allPizza[i].style.width = pizzaSize + "%";
    }
  }
...
// Make this declaraction of pizzasDiv variable outside of the For Loop seen below it.
...
function updatePositions() {
  ...
  // Move all var declarations outside of the For loop
  ...
  var halfWidth = window.outerWidth/2;

  // Assign column index to each mover instead of using item[i].style.left and basicLeft
  // The idea is to reduce recalc styles as much as possible.
  // This will be used to calc the transformX value with phase formula.
  // Please see readme.md for citings/reference.
  var myLeft = [];
  var colIdx = (Math.floor(window.outerWidth/256) + 1);
  for(var il = 0; il <= itemLength;) {
      for(var cc = 0; cc <= colIdx; cc++) {
         myLeft[il] = cc * 256;
         il++;
      }
  }
  ...
  for (var i = 0; i < itemLength; i++) {
    ...
    transX = ((myLeft[i] + 100 * phase) - halfWidth);
    // items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
    items[i].style.transform = "translateX(" + transX + "px)";
  }
...
// Use requestAnimationFrame to handle animation
window.addEventListener('scroll', function() {
    window.requestAnimationFrame(updatePositions);
});

// Generates the sliding pizzas when the page loads.
// Dynamic mover rows depending on height of screen
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  // Delcare all vars outside of the For loop.
  // Get window height and divide it by 200 which should easily give enough rows to fill the screen with pizzas
  // Use moverCount in the For loop count i <= moverCount if you want a dynamic number of pizza rows based on height.
  var moverCount = (Math.floor((window.outerHeight)/256)*cols);
  // Note: moverCount doesn't increase rendering speed you can use 32 pizzas will fill the screen.
  // We create var targetpizzaId outside of for loop and appendChild to it
  var elem;
  var targetPizzaId = document.getElementById("movingPizzas1");
  var s = 256;
  for (var i = 0; i <= moverCount; i++) {
    ...
    targetPizzaId.appendChild(elem);
  }
  updatePositions();
});
